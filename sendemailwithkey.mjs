...
export const processSendEmailWithKey = async (event) => {
    
    console.log('processSendEmailWithKey');
    ...
    try {
        ...
        serverkey = JSON.parse(event.body).serverkey.trim();
    } catch (e) {
    ...
    if(serverkey == null || serverkey == "" || serverkey.length <= 0) {
        const response = {statusCode: 400, body: {result: false, error: "Serverkey is not valid!"}}
       // processAddLog(userId, 'detail', event, response, response.statusCode)
        return response;
    }
